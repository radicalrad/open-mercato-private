...
- **Key decisions made:**
  - **DATABASE_URL fix:** corrected `${POSTGRES_USER:***...` → `${POSTGRES_USER}:${POSTGRES_PASSWORD}` in all three files. In dev, also dropped the `${POSTGRES_USER:-postgres}` / `${POSTGRES_PASSWORD:-postgres}` defaults on DATABASE_URL so a missing `.env` cannot silently authenticate with a guessed password. The dev compose's postgres service still uses those defaults for the postgres container itself (so it boots without `.env`), but the app's DATABASE_URL will fail closed if the user has not set real values.
